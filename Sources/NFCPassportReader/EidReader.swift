...
public class EidReader : NSObject {
    private typealias NFCCheckedContinuation = CheckedContinuation<DataGroupModel, Error>
    private var nfcContinuation: NFCCheckedContinuation?

    private var eid : NfcEidModel = NfcEidModel()
    private var dataModel : DataGroupModel = DataGroupModel()
    ...
    public func readEid( mrzKey : String, tags : [DataGroupId] = [], skipSecureElements : Bool = true, skipCA : Bool = false, skipPACE : Bool = false, customDisplayMessage : ((NfcViewDisplayMessage) -> String?)? = nil) async throws -> DataGroupModel {
        
        self.eid = NfcEidModel()
        self.dataModel = DataGroupModel()
        self.mrzKey = mrzKey
        self.skipCA = skipCA
        ...
        guard NFCNDEFReaderSession.readingAvailable else {///Kiểm tra xem có thể đọc thẻ NFC không.
            throw NfcEIdReaderError.NFCNotSupported ///Nếu không, ném ra lỗi
        }
        
        if NFCTagReaderSession.readingAvailable {
            readerSession = NFCTagReaderSession(pollingOption: [.iso14443], delegate: self, queue: nil) // khởi tạo đối tượng phiên đọc thẻ NFC
            
            self.updateReaderSessionMessage( alertMessage: NfcViewDisplayMessage.requestPresentEid )  /// cập nhật message theo chu trình đọc
            readerSession?.begin() // bắt đầu đọc thẻ ở đây
        }
...
extension EidReader : NFCTagReaderSessionDelegate {
    ...
    public func tagReaderSession(_ session: NFCTagReaderSession, didInvalidateWithError error: Error) {
        ...
        self.readerSession = nil // bắt các lỗi nFC khi đọc thẻ
        ...
        if let readerError = error as? NFCReaderError, readerError.code == NFCReaderError.readerSessionInvalidationErrorUserCanceled
            ...
            nfcContinuation?.resume(throwing: userError) // kết thúc tiến trình và show error nếu có
            nfcContinuation = nil
        }
    }
    ...
    public func tagReaderSession(_ session: NFCTagReaderSession, didDetect tags: [NFCTag]) {
        ...
        if tags.count > 1 {  // check loại thẻ đang đọc nhiều hơn một loại
        ...
        ///
        /// Connect với thẻ và check xem thẻ thuộc lại Tag nào có đúng loại Tag của thẻ căn cước hay không
        ///
        let tag = tags.first!
        var eIdTag: NFCISO7816Tag // đây là tiêu chuẩn quốc tế cho thẻ thông minh dành cho hộ chiếu điện tử, thẻ căn cước, và các ứng dung bảo mật cao khác
        switch tags.first! {
        case let .iso7816(tag): // nếu đúng thẻ này thì bắt đầu đọc còn không sẽ loại bỏ vì không đúng thẻ
            eIdTag = tag
        default:
        ...
        Task { [eIdTag] in
            do {
                /// Phương thức này thiết lập kết nối thẻ và kích hoạt thẻ. Kết nối với cùng một thẻ hiện đang mở không có hiệu lực.
                 /// Việc kết nối với một thẻ khác sẽ tự động chấm dứt kết nối thẻ trước đó và đưa nó vào trạng thái tạm dừng. Thẻ vẫn ở trong
                 /// trạng thái được kết nối cho đến khi thẻ khác được kết nối hoặc quá trình bỏ phiếu được khởi động lại.
                ...
                tagReader.progress = { [unowned self] (progress) in /// update tiến trình đọc lên Ui
...
extension EidReader {
    ...
    func startReading(tagReader : TagReader) async throws -> DataGroupModel {
    ...
    func readDataGroups( tagReader: TagReader ) async throws {
        ...
        for dgId in DGsToRead {
            ...
            if let dg = try await readDataGroup1(tagReader:tagReader, dgId:dgId) {
//                self.eid.addDataGroup( dgId, dataGroup:dg )
                switch dgId {
                      case .COM:
                          self.dataModel.COM = dg
                      case .SOD:
                          self.dataModel.SOD = dg
                      case .DG13:
                          self.dataModel.DG13 = dg
                      case .DG2:
                          self.dataModel.DG2 = dg
                      case .DG1:
                          self.dataModel.DG1 = dg
                     case .DG15:
                         self.dataModel.DG15 = dg
                     case .DG14:
                         self.dataModel.DG14 = dg
                      default:
                          // Xử lý cho các nhóm dữ liệu khác nếu có
                          print("Unhandled Data Group ID: \(dgId)")
                      }
    ...
    func readDataGroup1( tagReader : TagReader, dgId : DataGroupId ) async throws -> String?  {
        ...
        repeat {
            do {
                let response = try await tagReader.readDataGroup1(dataGroup:dgId)
