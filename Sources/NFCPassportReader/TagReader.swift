...
public class TagReader {
    ...
    func readDataGroup( dataGroup: DataGroupId ) async throws -> [UInt8]  {
        guard let tag = dataGroup.getFileIDTag() else {
            throw NfcEIdReaderError.UnsupportedDataGroup
        }
        
        return try await selectFileAndRead(tag: tag )
    }

    func readDataGroup1(dataGroup: DataGroupId) async throws -> String {
    guard let tag = dataGroup.getFileIDTag() else {
        throw NfcEIdReaderError.UnsupportedDataGroup
    }
    
    // Đọc dữ liệu dưới dạng mảng byte
    let byteArray = try await selectFileAndRead(tag: tag)
    
    // Chuyển đổi mảng byte thành Data
    let data = Data(byteArray)
    
    // Mã hóa Data thành chuỗi Base64
    ...
    func sendGeneralAuthenticate( data : [UInt8], lengthExpected : Int = 256, isLast: Bool) async throws -> ResponseAPDU {
        ...
        do {
            ...
            if case NfcEIdReaderError.ResponseError(_, let sw1, let sw2) = error,
               sw1 == 0x67, sw2 == 0x00 {
    ...
    func selectFileAndRead( tag: [UInt8]) async throws -> [UInt8] {
        ...
        guard let readHeaderCmd = NFCISO7816APDU(data:Data([0x00, 0xB0, 0x00, 0x00, 0x00, 0x00,0x04])) else {
            throw NfcEIdReaderError.UnexpectedError
        }
        ...
        while remaining > 0 {
            ...
            Log.debug( "TagReader - data bytes remaining: \(remaining), will read : \(readAmount)" )
            let cmd = NFCISO7816APDU(
                instructionClass: 00,
            ...
            Log.debug( "TagReader - got resp - \(binToHexRep(resp.data, asArray: true)), sw1 : \(resp.sw1), sw2 : \(resp.sw2)" )
            data += resp.data
    ...
    func send( cmd: NFCISO7816APDU ) async throws -> ResponseAPDU {
        ...
        if rep.sw1 != 0x90 && rep.sw2 != 0x00 {
            ...
            let tagError: NfcEIdReaderError
            if (rep.sw1 == 0x63 && rep.sw2 == 0x00) {
                tagError = NfcEIdReaderError.InvalidMRZKey
            } else {
                let errorMsg = self.decodeError(sw1: rep.sw1, sw2: rep.sw2)
                Log.error( "reason: \(errorMsg)" )
                tagError = NfcEIdReaderError.ResponseError( errorMsg, sw1, sw2 )
            }
            throw tagError
        }
